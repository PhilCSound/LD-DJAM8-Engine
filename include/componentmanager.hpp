...
template<typename T>
std::pair<bool, T>& ComponentManager::GetComponentPair(unsigned int index)
{
...
template<typename T>
void ComponentManager::AddComponent(const T& val, unsigned int index)
{
...
template<typename T>
void ComponentManager::RemoveComponent(unsigned int index)
{
	std::shared_ptr<ComponentArray<T>> ptr = GetComponentArray<T>();
	auto& pair = (*ptr)[index];
	pair.first = false;
